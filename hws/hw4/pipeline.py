...
class Pipeline:
    ...
    SMALL_GRID = { 
                ...
                'ET': {'n_estimators': [100, 10000], 'max_depth': [5,50], 'max_features': ['sqrt','log2'],
                       'min_samples_split': [2,10], 'n_jobs':[-1]},
                'AB': {'algorithm': ['SAMME', 'SAMME.R'], 'n_estimators': [1,10,100,1000,10000]},
                'GB': {'n_estimators': [100, 10000], 'learning_rate' : [0.001,0.1,0.5],
    ...
    def run(self, df, time_col, predictor_sets, label, start, end, test_window_months, 
        ...
        self.get_train_test_times(start, end, test_window_months, outcome_lag_days)
...
def compare_model_precisions_at_k(evaluations_filepath, k,  Ns=[], model_types=[], model_ids=[],
    save_output=False, output_filepath=None):
    '''
